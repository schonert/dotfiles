...
let g:colors_name = 'sschonert'

let s:gui_running = has('gui_running')

let g:sschonert_kill_italic = get(g:, 'sschonert_kill_italic', 0)
let g:sschonert_high_contrast = get(g:, 'sschonert_high_contrast',
            \ !s:gui_running && has('termguicolors') && &termguicolors ?
            \   ['cui'] : [])

let s:high_contrast =
    \ (s:gui_running && index(g:sschonert_high_contrast, 'gui') >= 0) ||
    \ (!s:gui_running && index(g:sschonert_high_contrast, 'cui') >= 0)

" Define reusable color palette.
let s:bg         = ['#06060f', 233]
let s:bgemphasis = ['#1f1e27', 235] " Sidebar
let s:bgstrong   = ['#536273', 238]
let s:fg         = ['#ffffff', s:high_contrast ? 255 : 255]
let s:hiddenfg   = ['#607080', 60]
let s:weakfg     = ['#999999', 103] " Comments
let s:weakerfg   = ['#555555', 102]
...
let s:green      = ['#beffaf', 150]
let s:darkgreen  = ['#5f8770', 65]
let s:skyblue    = ['#c8ddf6', 153]
...
function! s:hi(name, fg, bg, attr) abort
    ...
    if type(a:attr) != s:NUMBER_TYPE && !(g:sschonert_kill_italic && a:attr ==# 'italic')
        let attr =  'gui=' . a:attr
        if a:attr !=# 'italic'
